          ...
          <v-card-text align="center">
            <v-form ref="form">
              <div class="position-relative">
                <v-text-field max-width="400" v-model="nickname" label="닉네임"
                  :rules="[v => !!v || '필수 항목']"></v-text-field>
                <v-icon :color="nicknameDuplicateCheckColor" class="check-icon">
                  mdi-check-circle
                </v-icon>
              </div>
              ...
              <div class="d-flex justify-center">
                <v-btn class="submit-button mt-7" max-width="150" @click="checkNickNameDuplication">
                  계속
                </v-btn>
              </div>
            </v-form>
...
export default {
  data() {
    return {
      ...
      nicknameDuplicateCheckColor: 'grey',
    }
  },
  methods: {
    ...
    async checkNickNameDuplication() {
      const response = await this.requestNickNameDuplicationCheckToDjango(this.nickname)
      console.log("checkNicknameDuplication 응답", response)
      if (!response) {
        this.nicknameDuplicateCheckColor = 'success'
        this.checkPassword()
      } else {
        this.nicknameDuplicateCheckColor = 'error'
      }
    }
  },
  watch: {
