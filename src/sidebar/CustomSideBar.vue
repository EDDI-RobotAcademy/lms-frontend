  ...
  <div id="app">
    <nav id="nav-bar">
      ...
      <div id="nav-header">
        ...
        <label for="nav-toggle" @click="navtoogle">
          <span id="nav-toggle-burger"></span>
        </label>
        <hr v-if="this.showhrTag">
      </div>
      <div id="nav-content">
      ...
      <div id="nav-footer">
        ...
        <button v-if="!isAuthenticated" style="display: block; margin: 0 auto; text-align: center;"
          id="nav-footer" @click="goToLogin">LOGIN</button>
      </div>
    </nav>
  </div>
</template>
...
:root {
  ...
  /*--navbar-dark-primary: rgba(255, 211, 90,0.8);*/
  --navbar-dark-primary: rgba(227, 139, 41);
  /*사이드바 배경색*/
  --navbar-dark-secondary: rgb(255, 194, 111);
  /*하단 USER, hr선 색*/
  --navbar-light-primary: #f5f6fa;
  /*사이드바 이름(MENU BOARD) 글씨 색*/
...
#nav-bar {
  ...
  user-select: none;
  z-index: 3;
  /*APP.vue를 통해 각 페이지마다 연결했을 때 사이드바가 희미하게 보이면서 정상작동을 하지 않을때 z-index의 숫자를 높여야했음.*/
}
